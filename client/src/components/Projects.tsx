    ...
    <section id="projects" className="py-12 md:py-20 bg-muted/30">
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        <div className="text-center mb-8 sm:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold mb-3 sm:mb-4" data-testid="text-projects-heading">
            Featured Projects
          </h2>
        ...
        <div className="flex flex-wrap justify-center gap-4 sm:gap-6 max-w-4xl mx-auto">
          {filteredProjects.map((project, index) => {
            ...
            return (
              <Card
                key={index}
                className="relative overflow-hidden hover-elevate transition-all duration-300 group w-full sm:w-[calc(50%-12px)] max-w-md"
                data-testid={`card-project-${index}`}
              >
                <div className={`relative h-32 sm:h-40 bg-gradient-to-br ${project.gradient}`}>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <Icon className="w-12 h-12 sm:w-16 sm:h-16 text-primary/40 group-hover:scale-110 transition-transform duration-300" />
                  </div>
                  <div className="absolute inset-0 bg-gradient-to-t from-background/90 to-transparent"></div>
                </div>

                <div className="p-4 sm:p-6">
                  <h3 className="text-lg sm:text-xl font-semibold mb-2 sm:mb-3" data-testid={`text-project-title-${index}`}>
                    {project.title}
                  </h3>

                  <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4 line-clamp-3" data-testid={`text-project-description-${index}`}>
                    {project.description}
                  </p>
      ...
      <Dialog open={!!selectedProject} onOpenChange={() => setSelectedProject(null)}>
        <DialogContent 
          className="max-w-lg p-0 gap-0 overflow-hidden bg-background border-border max-h-[90vh] overflow-y-auto" 
        ...
        >
          {selectedProject && (
            <div className="flex flex-col">
              ...
              <div className="relative bg-gradient-to-br from-primary/30 via-primary/15 to-primary/5 p-4 sm:p-6 pb-6 sm:pb-8">
                <DialogHeader>
                  <DialogTitle className="text-xl sm:text-2xl font-bold pr-8" data-testid="text-modal-title">
              ...
              <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                <div>
                  <h4 className="text-xs sm:text-sm font-semibold mb-2 sm:mb-3 text-primary uppercase tracking-wider flex items-center gap-2">
                    <span className="w-6 sm:w-8 h-0.5 bg-primary rounded"></span>
                    About Project
                  </h4>
                  <p id="project-description" className="text-xs sm:text-sm leading-relaxed text-muted-foreground" data-testid="text-modal-description">
